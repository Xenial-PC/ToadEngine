...
using Disposable = ToadEngine.Classes.DearImGui.OpenTK.Extensions.Disposable;
...
using ObjectLabelIdentifier = OpenTK.Graphics.OpenGL.ObjectLabelIdentifier;
using OpenGLStateScope = ToadEngine.Classes.DearImGui.OpenTK.Extensions.OpenGLStateScope;
...
using SizedInternalFormat = OpenTK.Graphics.OpenGL.SizedInternalFormat;
...
using TriangleFace = OpenTK.Graphics.OpenGL.TriangleFace;
using Vector2 = System.Numerics.Vector2;
using VertexAttribType = OpenTK.Graphics.OpenGL.VertexAttribType;
using Win32Exception = PInvoke.Win32Exception;

// ReSharper disable InconsistentNaming

namespace ToadEngine.Classes.DearImGui.OpenTK;
...
public sealed class ImGuiController : Disposable
{
    ...
    public ImGuiController(GameWindow window, string? fontPath = null, int? fontSize = null, float? fontPixelSize = null)
    {
        ...
        using (new ImGuiContextScope(Context))
        {
            ...
            InitializeFont(fontPath, fontSize, fontPixelSize);
    ...
    private unsafe void InitializeFont(string? fontName, int? fontSize, float? fontPixelSize)
    {
        if (fontName != null)
        {
            fontPixelSize = fontPixelSize switch
            {
                null => 10.0f,
            ...
            var fontArray = RReader.ReadBytes(fontName);
            var pinnedArray = GCHandle.Alloc(fontArray, GCHandleType.Pinned);
            var fontPointer = pinnedArray.AddrOfPinnedObject();

            IO.Fonts.AddFontFromMemoryTTF(fontPointer, (int)fontSize, size, null, ref *ranges);
            pinnedArray.Free();
        }
        else
        {
